from config import round_pose, scale, cam_type, crop_Hor, crop_Ver, ouput_all_image
import os
import cv2
...
filename = os.listdir(ouput_all_image)
...
if len(filename) >= round_pose:
    ...
    print("round_poes must less than number of image")
...
for i in image_passed:
    if filename[i].endswith(".jpg") or filename[i].endswith(".png"):   
        image_path = os.path.join(ouput_all_image, filename[i])          
